...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="f:ImplementationGuide">
    ...
    <div>
      ...
      <xsl:if test="$globals='Y'">
        ...
        <h3>Global profiles</h3> {% include
    globals-table.xhtml %} </xsl:if>
      <xsl:for-each select="f:definition/f:grouping">
        <xsl:if test="not(@id = '-str-profile')">
          <xsl:variable name="relevantResources">
            ...
            parent::f:definition/f:resource[f:extension[@url='http://hl7.org/fhir/StructureDefinition/implementationguide-page']][f:package/@value=current()/@id or f:groupingId/@value=current()/@id][not(f:extension[@url='http://hl7.org/fhir/StructureDefinition/tools-alternateVersion'] or $version) or (f:extension[@url='http://hl7.org/fhir/StructureDefinition/tools-alternateVersion']/f:valueCode/@value=$version)]">
  ...
  <xsl:template match="f:grouping">
    ...
    {{ grouping_desc | markdownify}}</xsl:text>
      ...
      select="count(parent::f:definition/f:resource[f:groupingId/@value=current()/@id]/f:description/@value)!=0" />
      <table
      class="grid">
      <col style="width:20%" />
      <tbody>
        <xsl:for-each select="parent::f:definition/f:resource[f:groupingId/@value=current()/@id]">
          <tr>
            <td style="column-width:30%">
              <a
                href="{f:extension[@url='http://hl7.org/fhir/StructureDefinition/implementationguide-page']/f:valueUri/@value}"
                title="{f:reference/f:reference/@value}">
                <!--                  <xsl:text>&#160;&#160;&#x2022;&#160;&#160;</xsl:text>-->
                <xsl:choose>
...
{{ desc | markdownify}}</xsl:text>
  ...
  <xsl:template match="f:grouping" mode="first">
    <tr>
      <xsl:apply-templates select=".|following-sibling::f:grouping[position() &lt; $columns]"
        mode="next" />
